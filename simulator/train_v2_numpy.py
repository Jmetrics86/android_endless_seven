...
print("=== Starting V2 NumPy Neural Network Training (Multi-Deck + Card Features) ===")

dataset_file = "kaggle_dataset/v2_selfplay_dataset.csv"
...
with open(dataset_file, "r") as f:
    ...
    for row in reader:
        rows.append([
            ...
            float(row['card_has_activate']),
            float(row['opp_slot_power']),
            float(row['reward'])
        ])

data = np.array(rows, dtype=np.float32)
print(f"Loaded {len(data)} V2 decision samples from {dataset_file}")
...
print("\nV2 NumPy Neural Network Training Complete.")
...
print("Saved V2 trained weights to endless_ai_weights.json")
